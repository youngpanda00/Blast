...
export const usePromoCode = () => {
  ...
  useEffect(() => {
    ...
    const lsKey = `promo-dismissed:${code}`;
  ...
  const dismiss = () => {
    if (promo?.code) {
      localStorage.setItem(`promo-dismissed:${promo.code}`, "1");
    }
    setModalOpen(false);
    setDismissed(true);
  };

  return { promo, loading, error, modalOpen, setModalOpen, percent, timeLeft, dismiss };
};
